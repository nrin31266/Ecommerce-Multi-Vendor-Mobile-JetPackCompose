package com.nrin31266.ecommercemultivendor.domain.usecase.wishlist

import com.nrin31266.ecommercemultivendor.common.ResultState
import com.nrin31266.ecommercemultivendor.domain.dto.response.UserWishlistProductResponse
import com.nrin31266.ecommercemultivendor.domain.repo.Repo
import kotlinx.coroutines.flow.Flow
import javax.inject.Inject

class CheckUserWishlistUseCase @Inject constructor(private val repo: Repo) {
    operator fun invoke(productId: Long) : Flow<ResultState<UserWishlistProductResponse>> = repo.isUserWishlist(productId)
}